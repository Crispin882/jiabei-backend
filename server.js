...
app.post('/connect-report', express.json({ limit: '48kb' }), async (req, res) => {
  try {
    ...
    const sys = '你是「Jiabei 顶级英语口语私教」，请根据下面两个中国小学生的英语对话实录，生成一份面向孩子的「对话小报告」（用于家长查看和孩子回顾）。'
      + '请用简洁的中文、清晰的版式（用 <h2>/<h3>/<ul>/<li>/<p> 等普通 HTML 标签，不要写内联 style 以外的复杂样式；标题字号不要过大、段落紧凑，适配手机弹窗查看），包含：'
      ...
      + '5.给家长的话：一句如何陪孩子练习的建议。全程用中文，语气亲切、积极、具体。只输出 HTML 片段（不要写「对话小报告」这个总标题，前端已经显示；直接从「1. 对话概况」开始，不要 <html>/<body>）。';
